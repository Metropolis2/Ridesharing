...
if __name__ == "__main__":
    ...
    #find the utility of traveling alone, as a driver and as a passenger
    ...
    def cost_computation(merged_dict):
        ...
        for i in range(len(merged_dict)):
            ...
            a = merged_dict[i][2]['b']
            b = merged_dict[i][3]['beta']
            c = merged_dict[i][3]['gamma']
            ...
            #C_P = (ttime * a) + epsilon_P
            #C_D = -(a)*ttime + mu*(math.log(((mu/b)*(1-math.exp((b*(-ta+ttime+t0))/(mu))) - (mu/c)*(math.exp(-(((c)*(ttime-ta+t1))/(mu)))-1)))) + epsilon_D
            my_tuple = (id, origin, destination, ttime,  a,  b,  c, ta, ta0,  t0,  t1,  mu, u, utility, epsilon_D, epsilon_P)
            my_list.append(my_tuple)
    ...
    cost1_D = round((ttime[1][2] + ttime[4][7] + ttime[14][16]), 2)
